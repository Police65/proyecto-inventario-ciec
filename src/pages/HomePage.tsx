...
import { Inventario, SolicitudCompra, ProductoRezagado, UserProfile, Departamento, SolicitudCompraEstado, Empleado, SolicitudCompraDetalle, Producto } from '../types'; // Path relative to src/pages
...
export const HomePage: React.FC = () => {
      ...
      try {
        if (userProfile.rol === 'usuario') {
          ...
          const { data: allReqsData, error: allReqsError } = await supabase
            .from('solicitudcompra')
            .select('id, empleado_id, estado, departamento_id, descripcion, fecha_solicitud, empleado:empleado_id(nombre,apellido), departamento:departamento_id(nombre), detalles:solicitudcompra_detalle(*, producto:producto_id(id, descripcion))'); // Added id to producto select
          if (allReqsError) throw allReqsError;
          
          const rawReqs: any[] = allReqsData || [];
           const processedReqs: SolicitudCompra[] = rawReqs.map(req => ({
            ...req,
            empleado: req.empleado as Empleado | undefined,
            departamento: req.departamento as Departamento | undefined,
            detalles: (req.detalles || []).map((detalle: any) => ({
              ...detalle,
              producto: detalle.producto as Producto | undefined,
            })),
          }));
  ...
  if (!userProfile) return <div className="p-4">Perfil de usuario no disponible.</div>;
...
// Default export was removed, now it's a named export: export const HomePage
// If it's intended to be default, change to: export default HomePage;
