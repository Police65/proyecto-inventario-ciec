...
import { OPENROUTER_API_URL, OPENROUTER_API_KEY } from '../config'; // Assuming config is in src/
...
// Check against the actual key from environment variables, not a hardcoded placeholder
const isPlaceholderKey = (key: string | undefined): boolean => {
  if (!key) return true;
  // Add known placeholder patterns if necessary, but generally a non-empty key from .env is assumed to be valid.
  // For example, if your .env.example has "YOUR_OPENROUTER_API_KEY_HERE"
  // you might check for that specific string.
  return key === "YOUR_OPENROUTER_API_KEY_PLACEHOLDER_IF_ANY" || key.startsWith("sk-or-v1-placeholder"); 
};

const SITE_URL = typeof window !== 'undefined' ? window.location.origin : "https://requisoftware.example.com"; // Fallback for non-browser env
