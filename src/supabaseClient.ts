...
import { SUPABASE_URL, SUPABASE_ANON_KEY } from './config'; // Assuming config.ts is in the same src directory
import { Database, SolicitudCompra, SolicitudCompraDetalle, Producto } from './types'; // Assuming types.ts is in the same src directory

// Ensure environment variables are provided via config.ts which reads from import.meta.env
if (!SUPABASE_URL) {
  throw new Error("Supabase URL is not defined. Please check your .env file and config.ts.");
}
if (!SUPABASE_ANON_KEY) {
  throw new Error("Supabase Anon Key is not defined. Please check your .env file and config.ts.");
}
