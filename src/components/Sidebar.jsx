...
const Sidebar = ({ isVisible, onNewRequest, onTabChange }) => {
  return (
    <aside className={`bg-primary text-white min-vh-100 p-4 ${isVisible ? 'visible' : 'hidden'}`} style={{ width: '250px', position: 'fixed', top: '56px', left: 0, transition: 'left 0.3s' }}>
      ...
      <nav className="mt-5">
        <ul className="list-unstyled">
          <li className="mb-3"><a href="#" className="text-white d-block p-2 hover:bg-blue-500" onClick={() => onTabChange('home')}>Home</a></li>
          <li className="mb-3"><a href="#" className="text-white d-block p-2 hover:bg-blue-500" onClick={() => onTabChange('solicitudes')}>Solicitudes</a></li>
