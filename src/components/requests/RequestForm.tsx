...
import { supabase } from '../../supabaseClient'; // Path relative to src/components/requests
import { Producto } from '../../types'; // Path relative to src/components/requests
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      {!customRequest && (
        <div className="space-y-4">
          {products.map((productItem) => ( // Renamed 'product' to 'productItem' to avoid conflict with imported 'Producto'
            <div key={productItem.id} className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
              <div className="flex-grow">
                <label htmlFor={`product-${productItem.id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Producto
                </label>
                <select
                  id={`product-${productItem.id}`}
                  value={productItem.productId}
                  onChange={(e) => handleProductChange(productItem.id, 'productId', e.target.value)}
              ...
              <div className="w-24">
                <label htmlFor={`quantity-${productItem.id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Cantidad
                </label>
                <input
                  type="number"
                  id={`quantity-${productItem.id}`}
                  value={productItem.quantity}
                  onChange={(e) => handleProductChange(productItem.id, 'quantity', parseInt(e.target.value, 10) || 1)}
                  min="1"
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                  required={!customRequest}
                />
