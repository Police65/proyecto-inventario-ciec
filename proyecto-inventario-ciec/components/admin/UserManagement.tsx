            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleEmployeeSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><label htmlFor="nombre" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nombre <span className="text-red-500">*</span></label><input type="text" name="nombre" id="nombre" value={currentEmpleado.nombre || ''} onChange={handleInputChange} required className={`mt-1 ${inputFieldClasses}`} onInvalid={(e) => (e.target as HTMLInputElement).setCustomValidity('Por favor, ingrese el nombre.')} onInput={(e) => (e.target as HTMLInputElement).setCustomValidity('')}/></div>
                        <div><label htmlFor="apellido" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Apellido <span className="text-red-500">*</span></label><input type="text" name="apellido" id="apellido" value={currentEmpleado.apellido || ''} onChange={handleInputChange} required className={`mt-1 ${inputFieldClasses}`} onInvalid={(e) => (e.target as HTMLInputElement).setCustomValidity('Por favor, ingrese el apellido.')} onInput={(e) => (e.target as HTMLInputElement).setCustomValidity('')}/></div>
         ...
         <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black bg-opacity-60 p-4">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-md">
                ...
                <form onSubmit={handleProfileSubmit} className="space-y-4">
                    <div><label htmlFor="emailP" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email <span className="text-red-500">*</span></label><input type="email" name="email" id="emailP" value={profileFormData.email} onChange={handleProfileFormChange} required className={`mt-1 ${inputFieldClasses}`} onInvalid={(e) => (e.target as HTMLInputElement).setCustomValidity('Por favor, ingrese un correo electrónico.')} onInput={(e) => (e.target as HTMLInputElement).setCustomValidity('')}/></div>
                    {!isEditingProfile && <div><label htmlFor="passwordP" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Contraseña <span className="text-red-500">*</span></label><input type="password" name="password" id="passwordP" placeholder="Mínimo 6 caracteres" onChange={handleProfileFormChange} required minLength={6} className={`mt-1 ${inputFieldClasses}`} autoComplete="new-password" onInvalid={(e) => (e.target as HTMLInputElement).setCustomValidity('Por favor, ingrese una contraseña (mínimo 6 caracteres).')} onInput={(e) => (e.target as HTMLInputElement).setCustomValidity('')}/></div>}
